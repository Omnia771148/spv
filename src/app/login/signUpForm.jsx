...
export default function Home({ handleBacktoLogin }) {
  ...
  const termsRef = useRef(null); // Ref for auto-scrolling

  // Scroll to terms when OTP is verified
  useEffect(() => {
    if (otpVerified && termsRef.current) {
  ...
  const sendOtp = async (e) => {
    ...
    try {
      // Initialize reCAPTCHA just before use
      setupRecaptcha();
      ...
      // Crucial: Reset reCAPTCHA on error so user can try again
      if (recaptchaVerifierRef.current) {
        try {
          recaptchaVerifierRef.current.clear();
        } catch (e) {/* ignore */ }
      ...
      if (error.code === 'auth/invalid-app-credential') {
        msg += "This usually means the domain (localhost) is not authorized in Firebase Console.";
      ...
      {loading ? <Loading /> : (
        <form onSubmit={(e) => e.preventDefault()}>
          ...
          {!otpSent ? (
            ...
            <>
              <input type="text" placeholder="Enter OTP" autoComplete="one-time-code" onChange={(e) => setOtp(e.target.value)} className="form-control mb-2" /><br />
              <button onClick={verifyOtp} className="btn btn-success w-100">Verify OTP</button>
            </>
          ) : (
            <div ref={termsRef} className="mt-4 p-3 border rounded bg-light">
