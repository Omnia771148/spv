'use client';
  ...
  export default function Home({ handleBacktoLogin }) {
    ...
    useEffect(() => {
      ...
      return () => {
        // Optional: Clean up if component unmounts for good, 
        // but for dev HMR it's safer to keep the instance.
      };
    }, []);
    ...
    const sendOtp = async (e) => {
      ...
      try {
        ...
        if (error.code === 'auth/captcha-check-failed') {
          alert("Firebase Config Error: Hostname mismatch. Please add this domain/IP to Firebase Authorized Domains in the console.");
        ...
        if (window.recaptchaVerifier) {
          ...
          // It will re-init on next render/action or we can manually reload page if easier
          // For now, let's just clear it to prevent stale state
        }
      } finally {
